...
async def auto_purchase_saved_cart_after_topup(
    ...
    *,
    ...
    # CUSTOM-UI: «корзина-подарок» → завершаем подарок, а не покупку подписки
    if global_cart and global_cart.get('type') == 'custom_gift':
        try:
            from app.handlers.custom_gift import complete_gift_after_topup

            await complete_gift_after_topup(db, user, global_cart, bot=bot)
        except Exception as _kel_gift_err:
            logger.error('CUSTOM-UI: ошибка завершения подарка после пополнения', error=str(_kel_gift_err))
        return False
    if global_cart:
