...
# Статусные эмодзи-индикаторы (вкл/выкл): их НЕ вырезаем из текста кнопки, даже если у
# кнопки есть премиум-иконка. Премиумной может быть только одна ведущая иконка, поэтому
# хвостовой статус должен остаться обычным эмодзи — иначе он становится невидимым (как
# было у тумблеров уведомлений ``Истечение подписки: ✅``).
_BUTTON_STATUS_KEEP = frozenset({'✅', '❌'})
...
def _strip_mapped_emoji(text: str) -> str:
    ...
    Исключение — статусные ``✅/❌`` (``_BUTTON_STATUS_KEEP``): они несут смысл вкл/выкл.
    """
    out: list[str] = []
    i = 0
    n = len(text)
    while i < n:
        ch = text[i]
        if ch in EMOJI_MAP and ch not in _BUTTON_STATUS_KEEP:
            i += 1
