...
async def download_media(
    ...
    Requires a signed token: Telegram file_ids are long unguessable bearer tokens,
    but treating them as such on a public endpoint means a leaked file_id (logs,
    screenshots, shared URLs) becomes a permanent unrevocable read grant. The
    expiring signed token reduces blast radius — a leaked file_id alone is no
    longer enough, and the URL stops working after the token expires.
    """
