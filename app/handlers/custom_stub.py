"""CUSTOM-UI: заглушки кнопок фич без бэкенда (стратегия «полный визуальный клон»).

Контракт: мёртвые кнопки используют ``callback_data="custom_soon:<feature>"``,
где ``<feature>`` ∈ {stories, post, tiktok, create_vpn, worker_links, gifts, profile}.
Хендлер отвечает алертом «скоро» (текст — через locale-ключ ``CUSTOM_SOON_<FEATURE>``).
...
CUSTOM_SOON_DEFAULT = '🔧 Раздел в разработке — скоро появится.'


async def show_soon_stub(callback: types.CallbackQuery, db_user: User):
    """Алерт-заглушка для кнопок фич без бэкенда (callback ``custom_soon:<feature>``)."""
    try:
        data = callback.data or ''
        feature = data.split(':', 1)[1] if ':' in data else ''
        texts = get_texts(db_user.language)
        default = CUSTOM_SOON_DEFAULTS.get(feature, CUSTOM_SOON_DEFAULT)
        key = f'CUSTOM_SOON_{feature.upper()}' if feature else 'CUSTOM_SOON_DEFAULT'
        await callback.answer(texts.t(key, default), show_alert=True)
    except Exception as error:
        logger.debug('CUSTOM-UI: ошибка заглушки custom_soon', error=error)
        try:
            await callback.answer(CUSTOM_SOON_DEFAULT, show_alert=True)
        except Exception:
            pass


def register_handlers(dp: Dispatcher):
    dp.callback_query.register(show_soon_stub, F.data.startswith('custom_soon:'))
