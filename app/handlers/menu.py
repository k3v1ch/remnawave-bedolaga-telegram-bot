...
async def show_info_menu(
    ...
    privacy_policy = (
        await PrivacyPolicyService.get_active_policy(db, db_user.language)
        if is_visible_in_bot(settings.PRIVACY_POLICY_DISPLAY_MODE)
        else None
    )
    public_offer = (
        await PublicOfferService.get_active_offer(db, db_user.language)
        if is_visible_in_bot(settings.PUBLIC_OFFER_DISPLAY_MODE)
        else None
    )
